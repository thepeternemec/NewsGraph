...
/** Click Spark: a small burst of monochrome sparks wherever you click. */
export function clickSpark(count = 9, duration = 420): () => void {
  ...
  const loop = (now: number) => {
    ...
    for (const spark of sparks) {
      spark.life -= dt / duration;
      spark.x += spark.vx * (dt / 16);
      spark.y += spark.vy * (dt / 16);
      spark.vy += 0.05 * (dt / 16);
      ctx.strokeStyle = `rgba(255,255,255,${Math.max(0, spark.life) * 0.8})`;
      ctx.lineWidth = 1.2;
      ctx.beginPath();
